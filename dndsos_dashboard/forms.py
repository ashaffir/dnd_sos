...
class FreelancerUpdateForm(forms.ModelForm):
    ...
    active_hours = forms.ChoiceField(widget=forms.Select(
        attrs={'class': 'form-control'}), choices=Employee.ACTIVE_HOURS)
...
class BankDetailsForm(forms.Form):
    ...
    full_name_in_native_language = forms.CharField(
        max_length=100, required=False, widget=forms.TextInput(attrs={'class': 'form-control'}))
    name_on_the_account = forms.CharField(
    ...
    country = forms.ChoiceField(required=True, choices=COUNTRIES, widget=forms.Select(
        attrs={'class': 'form-control'}))
    phone_number = forms.CharField(
        max_length=100, required=False, widget=forms.TextInput(attrs={'class': 'form-control'}))

    # Bank Details
    iban = forms.CharField(max_length=100, required=True, widget=forms.TextInput(
        attrs={'class': 'form-control'}))
    swift = forms.CharField(max_length=100, required=False, widget=forms.TextInput(
        attrs={'class': 'form-control'}))
    account_number = forms.CharField(
        max_length=100, required=False, widget=forms.TextInput(attrs={'class': 'form-control'}))
    account_ownership = forms.BooleanField(
        required=True, widget=forms.CheckboxInput(attrs={'class': ''}))
