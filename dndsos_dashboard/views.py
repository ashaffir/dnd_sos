...
# Default distance beween business and available freelancers
DEFAULT_FREELANCER_RANGE = 1.0
...
def b_dashboard(request, b_id):
    ...
    daily_orders = Order.objects.filter(
        business=request.user.pk, created__contains=today)
    ...
    if request.user.relationships:
        context['num_active_freelancers'] = len(
            request.user.relationships['freelancers'])
...
def f_dashboard(request, f_id):
    ...
    if request.method == 'POST':
        if freelancer.is_delivering:
            messages.error(
                request, "Status can't be changed while in the process of delivery")
    ...
    if user_profile.credit_card_token:
        ...
        try:
            ...
            messages.error(request, gettext(
                'Failed getting credit card information. Please contact Pickndell support'))
    ...
    if request.method == 'POST':
        ...
        if 'update_profile' in request.POST:
            ...
            if request.POST.get("city_streets"):
                new_street = request.POST.get("city_streets").replace(
                    '\'', '').replace('\"', '')
            ...
            if request.POST.get("profileAddress"):
                ...
                try:
                    ...
                    try:
                        user_profile.location = Point(business_lat, business_lon) if platform.system(
                        ) == 'Darwin' else Point(business_lon, business_lat)
                    except Exception as e:
                        print(
                            f'>> VIEWS DASHBOARD: Failed getting business location. ERROR {e}')
                        logger.error(
                            f'>> VIEWS DASHBOARD: Failed getting business location. ERROR {e}')

                    print(
                        f'>> VIEWS DASHBOARD: business location: {user_profile.location} lat: {user_profile.lat} lon: {user_profile.lon}')
                    if not user_profile.location or not user_profile.lon or not user_profile.lat:
                        print('Failed to update business address')
                        messages.error(
                            request, 'This address is not valid please try again or a nearby location.')
                        return redirect(request.META['HTTP_REFERER'])
                except Exception as e:
                    logger.error(
                        f'Failed getting business location coordinates. ERROR: {e}')
            ...
            try:
                ...
                messages.success(
                    request, f'There was ann error processing your request. ERROR: {e}')
            ...
            try:
                ...
                if sent_sms_status:
                    ...
                    print(
                        f'>>>>>>> FAILE TO SEND SMS <<<<<<<< Error: {sent_sms_status}')
                    return render(request, 'dndsos_dashboard/failed-phone-verification.html')
            except Exception as e:
                print(
                    f">>> DASHBOARD VIEWS: Failed sending the SMS. ERROR: {e}")
                logger.error(
                    f">>> DASHBOARD VIEWS: Failed sending the SMS. ERROR: {e}")
                messages.error(request, gettext(
                    "Something went wrong with your phone verification. Please try later."))
                return redirect(request.META['HTTP_REFERER'])
...
def f_profile(request, f_id):
        ...
        if 'updateProfile' in request.POST:
            ...
            try:
                ...
                if sent_sms_status:
                    ...
                    print(
                        f'>>>>>>> FAILE TO SEND SMS to {phone} <<<<<<<< Error: {sent_sms_status}')
                    logger.error(
                        f'>>>>>>> FAILE TO SEND SMS <<<<<<<< Error: {sent_sms_status}')

                    return render(request, 'dndsos_dashboard/failed-phone-verification.html')
            except Exception as e:
                print(f">>> DASHBOARD VIEWS: Failed sending SMS. ERROR: {e}")
                logger.error(
                    f">>> DASHBOARD VIEWS: Failed sending SMS. ERROR: {e}")
                messages.error(
                    request, gettext("Something went wrong with your phone verification. Please try later"))
                return redirect(request.META['HTTP_REFERER'])
            ...
            try:
                ...
                user_profile.save()
                messages.success(
                    request, 'You have successfully updated a payment method to PayPal.')
                return redirect(request.META['HTTP_REFERER'])
            except EmailNotValidError as e:
                messages.error(
                    request, f'You have entered a non-valid email. Error: {e}')
                return redirect(request.META['HTTP_REFERER'])
            ...
            user_profile.save()
            messages.success(
                request, 'You have successfully updated a payment method to Phone Payments.')
            return redirect(request.META['HTTP_REFERER'])
            ...
            user_profile.save()
            messages.success(
                request, 'You have successfully set Paypal as a default payment method.')
            return redirect(request.META['HTTP_REFERER'])
        elif 'makePreferred_bank' in request.POST:
            user_profile.preferred_payment_method = 'Bank'
            user_profile.save()
            messages.success(
                request, 'You have successfully set bank transfer as a default payment method.')
            return redirect(request.META['HTTP_REFERER'])
        elif 'makePreferred_phone' in request.POST:
            user_profile.preferred_payment_method = 'Phone'
            user_profile.save()
            messages.success(
                request, 'You have successfully set phone-transfer as a default payment method.')
            return redirect(request.META['HTTP_REFERER'])
    ...
    # Check earnings due
    daily_orders = Order.objects.filter(
        Q(freelancer=request.user.pk) & Q(status='COMPLETED'))
    ...
    if request.method == 'POST':
        if 'rateFreelancer' in request.POST:
            ...
            if freelancer_rating:
                ...
                calculate_freelancer_total_rating(
                    order.freelancer.freelancer.pk)
...
def b_alerts_items(request, b_id):
    context = {}
    orders = Order.objects.filter(Q(business=b_id) & Q(
        status='REQUESTED') | Q(status='REJECTED') | Q(status='RE_REQUESTED'))
    context['orders'] = orders
...
def freelancers(request, b_id):
    ...
    completed_orders = Order.objects.filter(
        Q(business=request.user.pk) & Q(status='COMPLETED') & ~Q(status='SETTLED'))
    ...
    if request.method == 'POST':
        ...
        if 'filter' in request.POST:
            if vehicle and city:
                filtered_freelancers = Employee.objects.filter(
                    is_approved=True, vehicle=vehicle, city=city)
            elif vehicle and not city:
                filtered_freelancers = Employee.objects.filter(
                    is_approved=True, vehicle=vehicle)
            elif not vehicle and city:
                filtered_freelancers = Employee.objects.filter(
                    is_approved=True, city=city)
            else:
    ...
    all_freelancers = Employee.objects.filter(
    ...
    for freelancer in all_freelancers:
        try:
            ...
            range_to_freelancer = round(
                business_location.distance(freelancer_location) * 100, 3)
...
def freelancer_accept(request, fid, oid):
    ...
    try:
        ...
        logger.error(
            f"mail not sent -- Email configurations required. ERROR: {ex}")
        messages.error(
            request, f"mail not sent -- Email configurations required. ERROR: {ex}")
        ...
        if 'bankDetailsSubmit' in request.POST:
            try:
                if form.is_valid():
                    ...
                    freelancer.save()
                    messages.success(
                        request, "Bank details were updated successfully")
                    return redirect('dndsos_dashboard:f-profile', f_id=f_id)
                else:
                    for error in form.errors:
                        messages.error(
                            request, f"Error updating bank details. {error}")
    ...
    if request.method == 'POST':
        ...
        try:
            verification_status = phone_verify(
                request, action='verify_code', phone=phone, code=code)
            if verification_status == 'approved':
                ...
                print(
                    f'>>> DASHBOARD VIEWS: Failed Twilio verification approval. Error: {verification_status}')
                logger.error(
                    f'>>> DASHBOARD VIEWS: Failed Twilio verification approval. Error: {verification_status}')
                return render(request, 'dndsos_dashboard/failed-phone-verification.html')
        except Exception as e:
            print(f'>>> DASHBOARD:Failed verify the phone. Error: {e}')
            logger.error(f'>>> DASHBOARD: Failed verify the phone. Error: {e}')
            return render(request, 'dndsos_dashboard/failed-phone-verification.html')
    else:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if verification_status == 'approved':
                ...
                print(
                    f'>>> Failed verify the phone. Error: {verification_status}')
                return render(request, 'dndsos_dashboard/failed-phone-verification.html')
        except Exception as e:
...
def phone_verify(request, action, phone, code):
    ...
    if action == 'send_verification_code':
        ...
        try:
            ...
            print(
                f'>>> DASHBOARD: Fail sending the confirmation code to Twilio. ERROR: {e}')
            logger.error(
                f'>>> DASHBOARD: Fail sending the confirmation code to Twilio. ERROR: {e}')
            return False
        ...
        try:
            ...
            print(
                f"DASHBOARD: Failed verifying the code sent to Twilio. ERROR: {e}")
            logger.error(
                f"DASHBOARD: Failed verifying the code sent to Twilio. ERROR: {e}")
            return e
