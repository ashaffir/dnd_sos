    ...
    try:
        if 'is_approved' in update_fields:
            if instance.is_approved:
                print(
                    f'>>> CORE SIGNALS: User profile approved: {update_fields}')
                logger.info(
                    f'>>> CORE SIGNALS: User profile approved: {update_fields}')
                ...
                try:
                    ...
                    print(
                        f'>>> CORE SIGNALS: User profile language: {email_language}')
                    try:
                        email_template = EmailTemplate.objects.get(
                            name='account_approval', language=email_language)
                    except Exception as e:
                        print(
                            f">>> CORE SIGNALS: Failed getting email template for account approval using default. ERROR: {e}")
                        logger.info(
                            f">>> CORE SIGNALS: Failed getting email template for account approval using default. ERROR: {e}")
                        email_template = EmailTemplate.objects.get(
                            name='account_approval', language='en')
                    ...
                    logger.error(
                        f'Failed sending account approval to the user {instance}. ERROR: {e}')
        else:
