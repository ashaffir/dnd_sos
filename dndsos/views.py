...
def home(request):
    ...
    try:
        if request.LANGUAGE_CODE == 'he':
            context['subtitle'] = ContentPage.objects.get(name='subtitle',section='subtitle',active=True, language='Hebrew')
            ...
            context['subtitle'] = ContentPage.objects.get(name='subtitle', section='subtitle',active=True, language='English')
...
def c_addendum(request):
    ...
    try:
        context['c_addendum_he'] = ContentPage.objects.get(name='c_addendum_he').content
    except Exception as e:
